...
namespace Lexical_Analyzer_Libary.Classes
{
    public class SyntaxAnalyzer
    {
        ...
        private void ParseStatementSequence()
        {
            while (true)
            {
                ...
                if (_lexicalAnalyzer.CurrentLexem == Lexems.End ||
                    ...
                    _lexicalAnalyzer.CurrentLexem == Lexems.EndFor ||
        ...
        /// <summary>
        /// Разбирает оператор for, включая инициализацию, условие и тело цикла.
        /// </summary>
 
        private void ParseForLoop()
        {
            CheckLexem(Lexems.For);

            // Генерация меток
            string upperLabel = CodeGenerator.GenerateLabel(); // Метка для начала цикла
            string lowerLabel = CodeGenerator.GenerateLabel(); // Метка для конца цикла

            CheckLexem(Lexems.LeftPar);

            // Разбор инициализации цикла
            ParseStatement(); // Предполагаем, что это инициализация (например, a = 1)

            CheckLexem(Lexems.To); // Проверка на 'To'

            // Разбор условия цикла
            ParseExpression(); // Например, a < 10

            CheckLexem(Lexems.RightPar);

            CheckLexem(Lexems.Do); // Проверка на 'Do'

            // Генерация кода для условия цикла будет в ParseComparison() через lowerLabel
            CodeGenerator.AddLabel(upperLabel);

            // Разбор тела цикла
            ParseStatementSequence(); // Напр., Print(a); a = a + 1;

            CodeGenerator.AddInstruction($"jmp {upperLabel}");
            CodeGenerator.AddLabel(lowerLabel);

            CheckLexem(Lexems.EndFor); // Проверка на 'Endfor'
        }
        /// <summary>
